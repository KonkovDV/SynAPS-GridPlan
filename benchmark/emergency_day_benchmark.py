"""Synthetic emergency-restoration day: узел «Восточный».

A lab instance of mass damage on a 110 kV feed, restored in one day.
Names, ids, durations and topology are invented — not a dump from a
named grid operator and not a reconstruction of a live incident.
...
SEED = 20260818
T0 = datetime(2026, 8, 18, 6, 0, tzinfo=UTC)  # сутки с 06:00; дата — ось инстанса, не новость
HORIZON_HOURS = 24
...
def build_emergency_problem() -> GridPlanProblem:
    ...
    # --- Повреждённые активы (ночное массовое повреждение) -------------------
    vl110 = asset(
        "vl110",
        "Л-101",
    ...
    window("w-t2", t2, 13.0, 17.0, "ПЛ-0901-14")  # согласована накануне
    ...
    j_osm_t1 = job(
        "osm-t1",
        "Осмотр Т-1 (Восточная) после повреждения",
        t1,
        JobKind.INSPECTION,
        60,
        ["ps_110"],
    ...
    # --- Хвост 0,4 кВ: частный сектор -----------------------------------------
    job(
        "rem-04",
        "Восстановление ответвления 0,4 кВ (частный сектор)",
        tp22,
        JobKind.EMERGENCY,
        150,
        ["vl_04"],
    ...
    # --- Плановое ТО под замороженной заявкой ПЛ (согласована накануне) -------
    ...
    frozen = [
        FrozenAssignment(
            ...
            source="pl_window",
            frozen_reason="Заявка ПЛ-0901-14 согласована накануне аварийных суток",
            immutable=True,
            data_provenance="synthetic",
        )
    ]
    ...
    return GridPlanProblem(
        ...
        domain_attributes={
            ...
            "context": "Синтетические аварийные сутки по регламенту ликвидации; не данные сетевой",
...
def run() -> dict[str, Any]:
    ...
    # остальное (включая замороженную ПЛ-0901-14) обязано остаться неподвижным.
...
def render_md(r: dict[str, Any]) -> str:
    ...
    if repair_ok and slot_changes == 0:
        b_verdict = (
            ...
            "план стабильным (0 перемещений); замороженная заявка ПЛ-0901-14 не сдвинута "
        ...
        b_verdict = (
            ...
            f"заявка ПЛ-0901-14 не сдвинута ({b['frozen_windows_moved']} конфликтов), "
    ...
    return f"""# Аварийные сутки: восстановление узла «Восточный»
...
реконструкция живого филиала. Версия GridPlan {r["gridplan_version"]},
